...
{section name=id loop=$entries}
	...
	{else}
	   <td class="actioncell"><a href="admin.php?action=modify&amp;dn={$entries[id].dn|escape:"url"}">{t}Modify{/t}</a></td>
	   <td class="actioncell"><a href="admin.php?action=delete&amp;dn={$entries[id].dn|escape:"url"}">{t}Delete{/t}</a></td>
	{/if}
	</tr>
{/section}
</table>
