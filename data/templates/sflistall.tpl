...
{section name=id loop=$entries}
	<tr class="contentrow{cycle values="even,odd{/t}">
	   ...
	   <td class="contentcell">{$entries[id].kolabhomeserver|escape:"html{/t}</td>
	   <td class="contentcell">{$entries[id].foldertype|escape:"html{/t}</td>
	{if $entries[id].deleted neq "FALSE{/t}
	   <td class="actioncell" colspan="2">{t}Folder deleted, awaiting cleanup...{/t}</td>
	{else}
	   <td class="actioncell"><a href="sf.php?action=modify&amp;dn={$entries[id].dn|escape:"url{/t}">{t}Modify{/t}</a></td>
	   <td class="actioncell"><a href="sf.php?action=delete&amp;dn={$entries[id].dn|escape:"url{/t}">{t}Delete{/t}</a></td>
	{/if}
	</tr>
{/section}
</table>
