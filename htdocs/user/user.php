 ...
 * Copyright (c) 2004 Martin Konold erfrakon <martin.konold@erfrakon.de>
...
function fill_form_for_modify( &$form, $dn, &$ldap_object ) {
	...
	if( isset( $form->entries['kolabFreeBusyFuture_0'] ) ) {
		if( is_array( $ldap_object['kolabFreeBusyFuture'] ) ) $freebusyfuture = $ldap_object['kolabFreeBusyFuture'][0];
...
switch( $action ) {
	...
	case 'save':
		if( $form->isSubmitted() ) {
			if( !$form->validate() ) {
				if($action != "firstsave") fill_form_for_modify($form, $ldap_object);
				$form->setValues();
	...
	case 'kill':
		if (!$dn) {
			...
			aray_push($errors, _("Error: you need administrative permissions to delete users"));
		}
