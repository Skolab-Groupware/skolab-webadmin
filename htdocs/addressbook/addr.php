							...
							if (!$errors) {
								...
								if (strcmp($dn,$newdn) != 0) {
									...
									foreach( $distlists as $distlist ) {
										...
										$errors[] = sprintf(_("Addressbook entry DN could not be modified, distribution list <a href='".$params['skolab_webadmin_url']."/distributionlist/list.php?action=modify&dn=%s'>'%s'</a> depends on it. To modify this entry, first remove it from the distribution list."), urlencode($distlist['dn']), $dlcn );
									}
		...
		case 'kill':
			...
			if( !$errors ) foreach( $distlists as $distlist ) {
				...
				if( ldap_mod_del( $ldap->connection, $distlist['dn'], array('member' => $dn ) ) ) {
					...
					$errors[] = sprintf(_("Failure to remove addressbook entry from distribution list '%s', entry will not be deleted."), $dlmail);
					break;
				}
			}
			if (!$errors) {
