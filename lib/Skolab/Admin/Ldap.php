...
class SkolabLDAP {
	...
	public function dn_escape($val) {
		...
		$val = str_replace(',',    '\,', $val);
		$val = str_replace('+',    '\+', $val);
		$val = str_replace('"',    '\"', $val); // fix highlighting flaw in mcedit with another "
		$val = str_replace('<',    '\<', $val);
		$val = str_replace('>',    '\>', $val);
		$val = str_replace(';',    '\;', $val);
		$val = str_replace('#',    '\#', $val);
		...
		for ($i = 0; $i < strlen($string); $i++) {
			...
			if (ord($char) < 32) {
				...
				if (strlen($hex) == 1) $hex = '0'.$hex;
					$string = str_replace($char, '\\'.$hex, $string); // fix highlighting flaw in mcedit with this: '
				}
			}
		return $string;
	}
	...
	function unescape_dn_value($val)
	{
		...
		$val = str_replace('\,',    ',', $val);
		$val = str_replace('\+',    '+', $val);
		$val = str_replace('\"',    '"', $val); // another highlighting fix '
		$val = str_replace('\<',    '<', $val);
		$val = str_replace('\>',    '>', $val);
		$val = str_replace('\;',    ';', $val);
		$val = str_replace('\#',    '#', $val);
